...
export GRADLE_USER_HOME="$SCRIPT_DIR/.gradle-cache"

"$SCRIPT_DIR/gw" --no-daemon --refresh-dependencies build -x test

( cd "$SCRIPT_DIR/gateway"     && ../gw --no-daemon :gateway:run     -DKAFKA_BOOTSTRAP="$KAFKA_BOOTSTRAP" -DDATA_DIR="$DATA_DIR" ) &
sleep 2
( cd "$SCRIPT_DIR/chat"        && ../gw --no-daemon :chat:run        -DKAFKA_BOOTSTRAP="$KAFKA_BOOTSTRAP" -DDATA_DIR="$DATA_DIR" ) &
